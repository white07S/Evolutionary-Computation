; crc32c.asm
; NASM assembly implementation of CRC32C using a lookup table

section .data
    ; CRC32C Lookup Table (crc_c)
    crc_c:
...
section .text
    global generate_crc32c
    ; If you're on Windows, use:
    ; global generate_crc32c@16
    ; and adjust the symbol naming accordingly.

; Function: uint32_t generate_crc32c(const char *buffer, size_t length)
; Parameters:
;   RDI - buffer (const char *)
;   RSI - length (size_t)
; Returns:
;   EAX - CRC32C value (uint32_t)
generate_crc32c:
    push    rbp                 ; Prologue
    mov     rbp, rsp

    ; Initialize crc32 to ~0
    mov     ecx, 0xFFFFFFFF     ; crc32 = ~0
...
.loop_start:
    ...
    cmp     r8, rsi
    jge     .loop_end           ; if i >= length, exit loop

    ; Load buffer[i]
    movzx   eax, byte [rdi + r8]   ; Load buffer[i] into EAX and zero-extend

    ; Compute (crc32 ^ buffer[i]) & 0xFF
    xor     eax, ecx              ; crc32 ^ buffer[i]
    and     eax, 0xFF              ; & 0xFF to get the lower byte

    ; Load crc_c[(crc32 ^ buffer[i]) & 0xFF]
    lea     rdx, [crc_c + rax*4]   ; Calculate address: crc_c + (EAX * 4)
    mov     edx, [rdx]             ; Load crc_c[value] into EDX

    ; Compute crc32 >> 8
    shr     ecx, 8                 ; crc32 >> 8

    ; crc32 = crc_c[value] ^ (crc32 >> 8)
    xor     ecx, edx               ; Update crc32
...
.loop_end:
    ; Finalize crc32 by inverting all bits
    not     ecx                     ; ~crc32

    ; Move the result to EAX (lower 32 bits of RCX)
    mov     eax, ecx
